...
.EXAMPLE
    PS C:\> Get-asGroupMembership -Computer ThatMachine -Group Administrators

    Outputs the group members from ThatMachine

.EXAMPLE
    PS C:\> get-content computers.txt | Get-asGroupMembership -Group Administrators

    Outputs the group members from all systems listed in the file computers.txt

.EXAMPLE
    PS C:\> ".", "server1", "server2" | Get-asGroupMembership -Group Administrators

    Outputs the group members from the local system, server1, and server2

.EXAMPLE
    PS C:\> Get-asGroupMembership -Computer ThatMachine -Group Administrators -raw

    Outputs a PSObject for further modification or piping to additional cmdlets

.EXAMPLE
    PS C:\> Get-asGroupMembership -Computer ThatMachine -Group Administrators -picture

    Creates a .png of the data, named a la computer_group_date-time

.EXAMPLE
    PS C:\> Get-asGroupMembership -Group Administrators

    Outputs the group members from the current domain

.EXAMPLE
    PS C:\> Get-asGroupMembership -Domain MyADDomain -Group Administrators
